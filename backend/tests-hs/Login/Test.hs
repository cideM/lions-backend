        ...
        [ testCase "render form with message if email is missing" $ do
            withDB $ \conn -> do
              withoutLogging $ \logger -> do
                ...
                out <- T.decodeUtf8 . B.concat . LB.toChunks . simpleBody <$> withFormRequest "" (\r s -> do login conn logger signerKey saltSep sessionKey Production r s)
          ...
          testCase "render form with message if password is missing" $ do
            withDB $ \conn -> do
              withoutLogging $ \logger -> do
                (_, sessionKey) <- ClientSession.randomKey
                out <- T.decodeUtf8 . B.concat . LB.toChunks . simpleBody <$> withFormRequest "email=foo@bar.com" (\r s -> do login conn logger signerKey saltSep sessionKey Production r s)
                T.isInfixOf "Ungültige Kombination aus Email und Passwort" out @?= True,
          testCase "render form with message if credentials don't match" $ do
            withDB $ \conn -> do
              withoutLogging $ \logger -> do
                ...
                out <- T.decodeUtf8 . B.concat . LB.toChunks . simpleBody <$> withFormRequest "email=foo@bar.com&password=foo" (\r s -> do login conn logger signerKey saltSep sessionKey Production r s)
                T.isInfixOf "Ungültige Kombination aus Email und Passwort" out @?= True,
          testCase "returns encrypted session cookie upon successful login" $ do
            withDB $ \conn -> do
              withoutLogging $ \logger -> do
                ...
                out <- withFormRequest "email=foo@bar.com&password=foobar" (\r s -> do login conn logger signerKey saltSep sessionKey Production r s)
          ...
          testCase "can also verify firebase passwords" $ do
            withDB $ \conn -> do
              withoutLogging $ \logger -> do
                ...
                out <- withFormRequest "email=foo@bar.com&password=user1password" (\r s -> do login conn logger signerKey saltSep sessionKey Production r s)
                simpleStatus out @?= status302
                rows <- SQLite.query_ conn "select key from sessions"
                case rows of
                  [[storedSessionId :: T.Text]] -> do
