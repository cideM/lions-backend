...
infoBox :: Html () -> Html ()
infoBox msg = do
  div_ [class_ "d-flex align-items-center border rounded p-3"] $ do
    infoCircle
  ...
  where
    infoCircle =
      toHtmlRaw
        ( [i|
      <svg style="min-width: 32px; min-height: 32px;" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-info-circle" viewBox="0 0 16 16">
